...
- **2026-08-04 (자동 포지셔닝: Roll 을 0° 로 '제약' 하고 나머지만 최적화 — `--fix-roll`)**
  - **왜**: 실제 기록 로그 5개(`debug_logs/logs_for_optimization`)로 v2 를 돌리면 자유 해가
    ...
    않았다 — roll 은 shear 항 `sinR·sinP` 으로만 보이는 **약관측 파라미터**라, 표본이 적거나
    궤적이 일직선이면 좌우 스케일 오차·노이즈를 빨아들이는 쓰레기통이 된다. 그 값이
    `epl_config.json` 에 저장되면 `room_transform()` 이 좌우를 1.68배 늘려 방 좌표가 망가진다.
  - **사후에 roll 만 0 으로 덮어쓰면 안 된다**: 런타임이 쓰는 것은 저장된 자세 전체다
    (`room = pos + A⁻¹·local`, `A=U·Rot(−yaw)`). x·y·Yaw·Pitch 는 '전단이 있는' 해에 맞춰
    적합된 값이라, roll 만 0 으로 바꾸면 **적합된 변환 ≠ 적용되는 변환** 이 된다.
    → `refine_affine_ba_roll0()` 로 **모형 자체를 제약**해 재적합한다:
    `B(local→room) = Rot(yaw)·diag(s, g)`, `g=1/cosPitch` (센서당 미지수 6→4개).
    가우스-뉴턴 + Huber IRLS + 백트래킹 라인서치(초기값 = 자유 해) — numpy 만 쓴다.
  - **`g ≥ 1` 로 묶는 이유**(G_MIN=1.0): 저장은 `pitch=acos(1/g)` 이므로 `g<1`(=기준센서보다
    평평)이면 pitch 가 0° 로 clamp 되어 전방 스케일이 사라진다 → 또 '적합≠적용'.
  - **★ 상자제약은 clamp 가 아니라 활성집합(active set)으로 풀어야 한다** — 이번에 실제로 물린
    함정이다. 경계에 붙은 `g` 를 스텝에서 clamp 만 하면 **그 성분이 섞인 스텝 전체가 상승방향**
    이 되어 라인서치가 전부 실패하고 GN 이 3회에서 멈춘다(실측: `pia-1-2` 가 최적점에서
    **393mm·5.4°** 벗어난 채 종료, RMS 603 vs 달성가능 419mm). 지금은 경계에서 밖으로 밀리는
    `g` 의 **야코비 열을 빼고 최소제곱을 다시 푼다** → 독립 좌표하강과 동일한 해(비용 5.9261e8,
    RMS 418.96mm)에 도달한다. 자체검증 `I-d` 가 이 케이스(기준센서가 가장 숙여진 배치)를
    '지역최적인가' 로 검사한다 — 예전 구현에서는 FAIL(rms 186.6 vs 88.9mm).
  - **★ RMS 는 정확도가 아니다**(가장 헷갈리는 지점): 자유도가 많은 쪽이 항상 RMS 가 낮다.
    자체검증(노이즈 큰 로그 3개 풀링)에서 고정 해는 **위치오차 131→36mm** 로 좋아진다(RMS 는
    96→98mm). 실데이터는 331→419mm 로 오르지만 그게 나쁜 신호가 아니다. 그래서 경고는 RMS
    변화와 함께 **'자유 해가 쓰던 roll 각도'** 를 같이 보여주고 3분기로 안내한다 —
    **<5°**(=RMS 격차는 roll 탓이 아니다, `--free-roll` 로 되돌리지 말 것) / ±15°(비교해 볼 만함) /
    그 이상(노이즈 흡수였으니 고정이 맞다). 합성검증 35/35 에서 GT 기울어짐이 0 인데도 경고가
    떴고 그때 고정 해가 항상 GT 에 더 가까웠다 → '경고=자유 해로 돌아가라' 가 아니다.
  - **Pitch 0° 가 '추정값'인지 '경계 산물'인지 구분해 표시한다**: `pitch_conf='clamped'` →
    리포트에 `(경계·추정아님)`. 그리고 `--ref` 조언은 **그 해가 존재할 때만** 한다 —
    센서쌍 affine 의 `|det|` 은 게이지와 무관한 상대 전방스케일(`g_b/g_a`)이라 사이클 곱이 1
    이어야 하는데, 실측 로그는 **3.24배 모순**이다(`scale_consistency()`). 즉 어떤 `--ref` 를
    골라도 Pitch 는 확정되지 않는다 — 그럴 때는 재측정을 안내한다(x·y·Yaw 는 그대로 쓸 수 있다).
  - **적용 범위**: `run_auto_positioning_v2.sh` **와** `run_auto_positioning.sh` 둘 다
    `FIX_ROLL=1`. 두 스크립트가 **같은 `epl_config.json` 에 쓰기** 때문에 한쪽만 자유로 두면
    그 스크립트를 한 번 돌리는 순간 roll 이 다시 −53.5° 로 돌아간다. 되돌리려면 `FIX_ROLL=0`
    또는 `--free-roll`(스크립트 인자 뒤에 CLI 가 붙으므로 CLI 가 이긴다).
  - **`--no-refine` 과 함께 쓰면 고정 최적화는 생략된다** — 그때는 라벨도 '0°고정' 이 아니라
    예전대로 '(관측불가)' 로 표시하고 경고를 남긴다(하지 않은 최적화를 했다고 말하지 않기 위해).
    참고: `--no-refine` 자체는 원래부터 전 센서를 원점·0° 로 만드는 퇴화 경로다(쌍 affine 을
    쓴다는 도움말과 다름). 이번 변경과 무관한 기존 상태.
  - **주의 — 지금 저장돼 있는 `epl_config.json` 은 그 '사후 덮어쓰기' 상태다**:
    `pia-1-1` 이 `pitch 17.94° + roll 0.0°` 인데 이 x/y/Yaw/Pitch 는 roll −53.5° 를 전제로
    적합된 값이다(자유 해 출력과 일치). → `./run_auto_positioning_v2.sh` (dry-run 없이)로
    다시 저장해야 일관된다. `.bak` 이 자동 생성된다.
  - **데이터 품질**: 이 로그들은 센서쌍 궤적이 거의 일직선(minor 241~287mm)이라 Pitch/Roll
    정보가 애초에 빈약하다. 겹침 구역에서 **곡선으로** 걸은 로그로 다시 기록하면 좋아진다.
